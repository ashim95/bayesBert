...
from .modeling_flax_bayes_bert import FlaxBayesLinear
...
class FlaxBertForSequenceClassificationModule(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.bayesian_classifier:
            self.classifier = FlaxBayesLinear(
                                features=self.config.num_labels,
                                prior_distribution=self.config.classifier_prior,
                                prior_params=self.config.classifier_prior_params,
                                dtype=self.dtype
            )
        else:
    ...
    def __call__(
        ...
        logits = self.classifier(pooled_output, sub_key, 10)
